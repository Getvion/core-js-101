...
function extractNameFromTemplate(str) {
  ...
  const lastName = str.split(' ')[2].replace('!', '');
...
function getRectangleString(width, height) {
  ...
  const preMiddle = `│${' '.repeat(width - 2)}│\n`;
  const middle = preMiddle.repeat(height - 2);
...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Z]/gi, (letter) => decode[alfabet.indexOf(letter)]);
}
