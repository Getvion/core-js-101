...
function getSumBetweenNumbers(n1, n2) {
  const arrOfNumbersBetween = [];

  for (let index = n1; index <= n2; index += 1) {
    arrOfNumbersBetween.push(index);
  }

  return arrOfNumbersBetween.reduce((acc, curr) => acc + curr);
}
...
function isTriangle(a, b, c) {
  // сумма двух сторон должна быть больше чем 3
  const firtSide = a + b > c;
  const secondSide = a + c > b;
  const thirdSide = b + c > a;

  if (firtSide && secondSide && thirdSide) return true;
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1Width > rect2.left && rect1Height > rect2.top) return true;
...
function isInsideCircle(circle, point) {
  ...
  // formula: https://math.stackexchange.com/questions/198764/how-to-know-if-a-point-is-inside-a-circle
...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    const beforeChar = str.slice(0, i);

    if (!str.includes(char, i + 1) && !beforeChar.includes(char)) return char;
  }
  return null;
}
